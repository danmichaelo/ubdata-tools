...
declare function local:signaturesAsDdc( $sigs as element()* )
as element()*
{
	for $sig in $sigs
...
declare function local:signaturesAsUdc( $sigs as element()* )
as element()*
{
	for $sig in $sigs
...
declare function local:label( $post as element() )
as xs:string
{
...
declare function local:uriFromTermId( $uri_base as xs:string, $termId as xs:string )
as xs:string
{
...
declare function local:posts( $posts as element()*, $scheme as xs:string, $uri_base as xs:string, $signature_handler as xs:string)
as element()*
{
	<skos:ConceptScheme rdf:about="{ $scheme }">
	{
		...
		return
		{
			if ($post/toppterm-id/text() = $post/term-id/text()) then
				<skos:hasTopConcept rdf:resource="{ local:uriFromTermId( $uri_base, $post/term-id/text() ) }"/>
			else ()
		}
	}
	</skos:ConceptScheme>,
	...
	return
	{
		if ( $post/se-id or $post/gen-se-henvisning ) then 
		{
			...
			else 
			{
				...
				return  <skos:Concept rdf:about="{ local:uriFromTermId( $uri_base, $seId ) }">
							<skos:altLabel xml:lang="nb">{ local:label( $post ) }</skos:altLabel>
						</skos:Concept>
			}
		}
		else
			<skos:Concept rdf:about="{ local:uriFromTermId( $uri_base, $post/term-id/text() ) }">
				...
				}</dct:modified>
				{
					...
					switch ($signature_handler)
						case "ddc" return local:signaturesAsDdc( $post/signatur )
						case "udc" return local:signaturesAsUdc( $post/signatur )
						default return ()
				}{
					...
					return <skos:broader rdf:resource="{ local:uriFromTermId( $uri_base, $x ) }"/>
				}{
					for $x in $post/ox-id/text()
					return <skos:broader rdf:resource="{ local:uriFromTermId( $uri_base, $x ) }"/>
				}{
					for $x in $post/se-ogsa-id/text()
					return <skos:related rdf:resource="{ local:uriFromTermId( $uri_base, $x ) }"/>
				}
			</skos:Concept>
	}
};
...
declare function local:toRdf( $posts as element()*, $scheme as xs:string, $uri_base as xs:string, $signature_handler as xs:string )
as element()*
{
	...
	{ 
		local:posts($posts, $scheme, $uri_base, $signature_handler)
	}
	</rdf:RDF>
};
